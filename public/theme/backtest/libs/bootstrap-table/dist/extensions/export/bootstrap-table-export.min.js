      ...
      if (!g.length) {
        g = a(
          [
            ...
            '<button class="btn' +
              b(" btn-%s", this.options.buttonsClass) +
              b(" btn-%s", this.options.iconSize) +
        ...
        if ("string" == typeof this.options.exportTypes) {
          var j = this.options.exportTypes
            .slice(1, -1)
            .replace(/ /g, "")
            .split(",");
          (i = []),
          ...
          h.find("li").click(function () {
            var b = a(this).data("type"),
              c = function () {
                ...
                d.$el.tableExport(
                  a.extend({}, d.options.exportOptions, { type: b, escape: !1 })
                ),
                  d.options.exportFooter && d.load(c);
              },
              e = d.header.stateField;
            if ("all" === d.options.exportDataType && d.options.pagination)
              d.$el.one(
                "server" === d.options.sidePagination
                  ? "post-body.bs.table"
