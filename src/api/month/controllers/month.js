...
async function insertManyMonth(ctx){

    let req = ctx.request

    const knex = strapi.db.connection;
    
    await insertOrUpdate(knex,'months',req.body);
    
    return 'Hello World!'
}

async function findMonth(ctx){
    ...
    if (end !="0"){
        var result = await strapi.db.query('api::month.month').findMany({
...
module.exports = {
    ...
    async updates(ctx) {
        return insertManyMonth(ctx)
    },
    async find(ctx){
        return findMonth(ctx)
    },
