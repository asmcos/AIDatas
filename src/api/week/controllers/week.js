...
async function insertManyWeek(ctx){

    let req = ctx.request

    const knex = strapi.db.connection;
    
    await insertOrUpdate(knex,'weeks',req.body);
    
    return 'Hello World!'
}

async function findWeek(ctx){
    ...
    if (end !="0"){
        var result = await strapi.db.query('api::week.week').findMany({
